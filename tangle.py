...
class Tangle(object):
# Modified by Xun, Jun-08-2022
    def __init__(self, rate=50, alpha=0.001, tip_selection='mcmc', nTipSelected=2, plot=False):
        self.time = 1.0
        self.rate = rate
        self.alpha = alpha
        self.nTips = nTipSelected # added to configure # of tip selections
    ...
    def next_transaction(self):
        ...
        dt_time = -math.log(1.0 - random.random()) / self.rate
        self.time += dt_time
        self.arrivalTimes.append(self.time)
        self.count += 1
        ...
        if len(approved_tips) >= self.nTips-1:
            print('\n\n >>>> Selected Tips: ', approved_tips)
    ...
    def mcmc(self):
        num_particles = self.nTips
        # print('Required Tips#: ', num_particles)
...
#            distances[tip] = distance
        ...
        tips = self.tip_walk_cache[:self.nTips]
        self.tip_walk_cache = list()
